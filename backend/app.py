...
def api_info():
    ...
    return jsonify({
        ...
        "endpoints": {
            ...
            "合规": ["POST /api/compliance/audit/text", "POST /api/compliance/audit/image", "GET/POST /api/compliance/rules"],
            "视觉": ["POST /api/vision/analyze", "POST /api/vision/aesthetic-match"],
            "知识库": ["GET/POST /api/knowledge", "POST /api/knowledge/generate-copy"],
...
def seed_initial_data():
    ...
    corpus_seed = [
        # --- 形制类 (10条) ---
        ("马面裙", "形制", "明代流行的裙装形制，前后有平幅裙门，两侧打褶", 
         "最具辨识度的汉服单品，非'马面'之意，源于裙门形似马面城墙垛口",
         json.dumps(["明制","裙装","标志性"], ensure_ascii=False),
         T("Mamian Qun (Horse-Face Skirt)","馬面裙（ばめんぐん）","마면군","Falda Mamian","Jupe Mamian","تنورة ماميان")),
        ("云肩", "形制", "披在肩部的装饰性衣饰，多为圆形或云朵形",
         "云肩象征吉祥如意，唐代已有雏形，明清盛行",
         json.dumps(["肩饰","装饰","明制"], ensure_ascii=False),
         T("Cloud Collar (Yunjian)","雲肩（うんけん）","운견","Cuello Nube","Col Nuage","ياقة السحابة")),
        ("补服", "形制", "明清官员官服，前胸后背缀有补子表示品级",
         "补子图案严格按品级：文官绣禽、武官绣兽",
         json.dumps(["官服","明清"], ensure_ascii=False),
         T("Mandarin Square Robe","補服（ほふく）","보복","Toga Mandarina","Robe Mandarine","رداء الماندرين")),
        ("交领", "形制", "汉服基本领型，左右衣襟交叉于胸前",
         "交领右衽是汉服核心特征，区别于少数民族左衽",
         json.dumps(["领型","基础","通用"], ensure_ascii=False),
         T("Cross-Collar (Jiaoling)","交領（こうりょう）","교령","Cuello Cruzado","Col Croisé","ياقة متقاطعة")),
        ("齐胸襦裙", "形制", "唐代流行的高腰裙装，裙腰系于胸上",
         "唐代女性典型装束，搭配大袖衫或半臂，展现盛唐气象",
         json.dumps(["唐制","女装","高腰"], ensure_ascii=False),
         T("High-Waist Ruqun","斉胸襦裙（せいきょうじゅくん）","제흉유군","Falda Alto Pecho","Jupe Haute Poitrine","تنورة عالية الصدر")),
        ("褙子", "形制", "宋代流行的直领对襟长外套，两侧开衩",
         "宋代女子常服，简约优雅，体现宋人'清雅'审美",
         json.dumps(["宋制","外套","女装"], ensure_ascii=False),
         T("Beizi (Side-Slit Robe)","褙子（はいし）","배자","Chaqueta Beizi","Veste Beizi","سترة بيزي")),
        ("直裰", "形制", "明代男子日常服装，交领长袍，两侧开衩",
         "直裰为明代士人常服，儒雅简约",
         json.dumps(["男装","明制","士人"], ensure_ascii=False),
         T("Straight Robe (Zhiduo)","直綴（じきてつ）","직철","Túnica Recta","Robe Droite","رداء مستقيم")),
        ("圆领袍", "形制", "圆领窄袖长袍，隋唐至明代的常服/官服",
         "唐代圆领袍影响深远，日本狩衣即源于此",
         json.dumps(["通用","官服","隋唐"], ensure_ascii=False),
         T("Round-Collar Robe","円領袍（えんりょうほう）","원령포","Túnica Cuello Redondo","Robe Col Rond","رداء بياقة مستديرة")),
        ("袄裙", "形制", "明代女子上袄下裙的搭配，袄为短上衣",
         "袄裙为明制汉服代表性搭配，袄有交领/竖领之分",
         json.dumps(["明制","女装","套装"], ensure_ascii=False),
         T("Aoqun (Jacket & Skirt)","襖裙（おうくん）","오군","Conjunto Aoqun","Ensemble Aoqun","طقم أوكون")),
        ("披风", "形制", "明代男女皆可穿着的直领对襟外套",
         "披风形似褙子但更宽大，为明代常见外搭",
         json.dumps(["明制","外套","通用"], ensure_ascii=False),
         T("Pifeng Cape","披風（ひふう）","피봉","Capa Pifeng","Cape Pifeng","عباءة بيفنغ")),

        # --- 纹样类 (6条) ---
        ("云纹", "纹样", "象征高升和如意的传统纹样，形似祥云",
         "云纹起源商周青铜器，汉代广泛用于织物",
         json.dumps(["吉祥纹样","常用","基础"], ensure_ascii=False),
         T("Cloud Pattern","雲紋（うんもん）","운문","Patrón de Nubes","Motif Nuage","نمط السحابة")),
        ("缠枝莲", "纹样", "莲花与枝蔓交织的连续纹样，寓意生生不息",
         "受佛教影响，莲花为圣洁象征，广泛应用于丝绸",
         json.dumps(["吉祥纹样","佛教","连续"], ensure_ascii=False),
         T("Lotus Scroll Pattern","蓮華唐草文（れんげからくさもん）","연화당초문","Patrón Loto Enredadera","Motif Lotus Entrelacé","نمط اللوتس المتشابك")),
        ("海水江崖", "纹样", "波涛与山崖组合的纹样，象征江山永固",
         "明清官服常用，寓意'福山寿海'、'江山一统'",
         json.dumps(["官服纹样","明清","吉祥"], ensure_ascii=False),
         T("Wave & Cliff Pattern","海水江崖文（かいすいこうがいもん）","해수강애문","Patrón Olas y Acantilados","Motif Vagues et Falaises","نمط الأمواج والمنحدرات")),
        ("龙凤纹", "纹样", "龙与凤组合的尊贵纹样，象征帝王与皇后",
         "龙纹在明清仅限皇室使用，民间使用蟒纹替代",
         json.dumps(["尊贵","皇家","明清"], ensure_ascii=False),
         T("Dragon & Phoenix Pattern","竜鳳文（りゅうほうもん）","용봉문","Patrón Dragón y Fénix","Motif Dragon et Phénix","نمط التنين والعنقاء")),
        ("牡丹纹", "纹样", "以牡丹花为题材的纹样，象征富贵荣华",
         "牡丹为'花中之王'，唐代起即受追捧，宋元明清广泛使用",
         json.dumps(["花卉","富贵","吉祥"], ensure_ascii=False),
         T("Peony Pattern","牡丹文（ぼたんもん）","모란문","Patrón de Peonía","Motif Pivoine","نمط الفاوانيا")),
        ("八宝纹", "纹样", "道教/佛教八种法宝组合纹样，寓意吉祥",
         "常见有宝伞、金鱼、宝瓶、莲花、法螺、吉祥结、宝幢、法轮",
         json.dumps(["宗教","吉祥","组合"], ensure_ascii=False),
         T("Eight Treasures Pattern","八宝文（はっぽうもん）","팔보문","Patrón Ocho Tesoros","Motif Huit Trésors","نمط الكنوز الثمانية")),

        # --- 工艺类 (4条) ---
        ("缂丝", "工艺", "通经断纬的高级丝织工艺，可织出精细图案",
         "缂丝宋代达顶峰，'一寸缂丝一寸金'，为非遗",
         json.dumps(["非遗","丝织","高级"], ensure_ascii=False),
         T("Kesi Silk Tapestry","綴織（けし）","극사","Tapiz Kesi","Tapisserie Kesi","نسيج كيسي")),
        ("刺绣", "工艺", "用针线在织物上绣制图案的传统工艺",
         "四大名绣：苏绣、湘绣、粤绣、蜀绣，各有特色",
         json.dumps(["非遗","手工","装饰"], ensure_ascii=False),
         T("Embroidery","刺繍（ししゅう）","자수","Bordado","Broderie","تطريز")),
        ("织锦", "工艺", "用彩色丝线织出花纹的高级丝织品",
         "南京云锦、成都蜀锦、苏州宋锦并称三大名锦",
         json.dumps(["丝织","高级","多彩"], ensure_ascii=False),
         T("Brocade","錦（にしき）","직금","Brocado","Brocard","ديباج")),
        ("蜡染", "工艺", "用蜡防染的印染工艺，图案独特",
         "苗族蜡染为国家级非遗，蓝白二色为经典配色",
         json.dumps(["非遗","印染","少数民族"], ensure_ascii=False),
         T("Batik","蝋纈（ろうけつ）","납염","Batik","Batik","باتيك")),

        # --- 礼仪类 (2条) ---
        ("右衽", "礼仪", "衣襟向右掩的穿着方式，为汉服核心特征",
         "右衽为华夏衣冠标志，孔子赞管仲'微管仲，吾其被发左衽矣'",
         json.dumps(["基础","标志性","礼仪"], ensure_ascii=False),
         T("Right Lapel (Youren)","右衽（うじん）","우임","Solapa Derecha","Revers Droit","طية يمنى")),
        ("冠礼", "礼仪", "古代汉族男子成年礼，加冠以示成人",
         "冠礼为'礼之始也'，周代已形成完整仪式，为儒家六礼之首",
         json.dumps(["成年礼","礼仪","周礼"], ensure_ascii=False),
         T("Capping Ceremony (Guanli)","冠礼（かんれい）","관례","Ceremonia de Coronación","Cérémonie du Chapeau","مراسم التتويج")),
    ]
    ...
    rules_seed = [
        # 原有6条
        ("沙特阿拉伯", "中东", "宗教禁忌", 
         json.dumps(["猪","猪肉","猪皮","酒精","酒类","十字架","佛像"], ensure_ascii=False),
         None, "高风险",
        ...
        # 新增12条
        ("阿联酋", "中东", "宗教禁忌",
         json.dumps(["裸体","比基尼","短裙","酒类广告","猪","狗"], ensure_ascii=False),
         "伊斯兰教对裸露和酒精有严格限制，女性着装需遮蔽",
         "高风险",
         "阿联酋保守伊斯兰文化对裸露内容和酒精元素零容忍",
         "确保服装遮蔽度足够，避免酒类与裸露元素"),
        ("韩国", "东亚", "文化冒犯",
         json.dumps(["韩服","kimchi","泡菜起源中国","端午节"], ensure_ascii=False),
        ...
        ("俄罗斯", "东欧", "政治敏感",
         json.dumps(["苏联","共产主义","乌克兰旗帜","克里米亚"], ensure_ascii=False),
         None, "高风险",
         "涉及前苏联及近现代政治符号在俄乌冲突背景下高度敏感",
         "避免任何政治符号，专注于服饰美学本身"),
        ("巴西", "拉美", "宗教禁忌",
         json.dumps(["耶稣受难","圣母像过度暴露","十字架亵渎"], ensure_ascii=False),
         "天主教国家，宗教符号需尊重使用",
         "中风险",
        ...
        ("意大利", "欧洲", "文化冒犯",
         json.dumps(["意大利面诋毁","披萨诋毁","黑手党刻板印象"], ensure_ascii=False),
         None, "低风险",
    ...
    kb_seed = [
        ("形制", "马面裙的历史与文化",
         ...
         json.dumps({"en":"The History of Mamian Qun","ja":"馬面裙の歴史","ko":"마면군의 역사"}, ensure_ascii=False),
         None,
         json.dumps({"北美":"如维多利亚时期裙撑的东方版本","日韩":"与韩服赤古里裙的东方共鸣","欧洲":"东方版的宫廷大裙摆"}, ensure_ascii=False)),
        ...
        ("工艺", "缂丝技艺 — 一寸缂丝一寸金",
         ...
         json.dumps({"en":"Kesi: The Pinnacle of Silk Art","ja":"綴織—究極の絹芸術","ko":"극사—비단 예술의 정점"}, ensure_ascii=False),
         None,
         json.dumps({"欧洲":"堪比欧洲哥白林挂毯工艺","日韩":"与西阵织并称东亚丝织双璧","中东":"如波斯地毯般匠心独运"}, ensure_ascii=False)),
        ...
        ("纹样", "云纹的千年演变",
         ...
         json.dumps({"en":"The Evolution of Cloud Patterns","ja":"雲紋の千年の歩み","ko":"운문의 천년 변천"}, ensure_ascii=False),
         None,
         json.dumps({"欧洲":"如巴洛克卷草纹般华丽","北美":"东方极简主义的纹样源头","东南亚":"与蜡染云纹异曲同工"}, ensure_ascii=False)),
        ...
        ("礼仪", "汉服右衽的文化密码",
         ...
         json.dumps({"en":"The Cultural Code of Right Lapel","ja":"右衽の文化的暗号","ko":"우임의 문화적 암호"}, ensure_ascii=False),
         None,
         json.dumps({"欧洲":"如西装左襟在上般的着装礼仪","日韩":"与和服左前相同的东亚衣冠传统","中东":"如阿拉伯长袍右掩般的文化身份"}, ensure_ascii=False)),

        ("形制", "齐胸襦裙与盛唐气象",
         "齐胸襦裙是唐代女性最具代表性的装束之一，裙腰高系至胸部以上，搭配短襦或大袖衫，展现盛唐时期的开放与自信。唐代丝绸之路繁荣，胡风影响显著，使得唐代女装呈现出色彩艳丽、款式多样的特点。齐胸襦裙在当代汉服复兴中极受欢迎，是展现大唐风华的首选形制。",
         json.dumps({"en":"High-Waist Ruqun and Tang Splendor","ja":"斉胸襦裙と盛唐の気象","ko":"제흉유군과 성당의 기상"}, ensure_ascii=False),
         None,
         json.dumps({"欧洲":"如帝政风格高腰裙般的优雅","北美":"堪比1920年代Flapper Dress的文化解放","中东":"如卡夫坦长袍般的飘逸感"}, ensure_ascii=False)),
        ...
        ("纹样", "龙凤纹的尊贵象征",
         ...
         json.dumps({"en":"Dragon and Phoenix: Symbols of Nobility","ja":"竜鳳文の尊貴なる象徴","ko":"용봉문의 존귀한 상징"}, ensure_ascii=False),
         None,
         json.dumps({"欧洲":"如皇室纹章般的身份象征","日韩":"与日本菊花纹章同为皇室专属","中东":"如阿拉伯书法艺术般的神圣感"}, ensure_ascii=False)),

        ("工艺", "四大名绣 — 苏湘粤蜀",
         "中国刺绣历史悠久，苏绣、湘绣、粤绣、蜀绣并称四大名绣。苏绣以精细雅洁著称，双面绣为其绝技；湘绣以狮虎题材见长，鬅毛针法独特；粤绣色彩富丽，金银线垫绣立体感强；蜀绣针法多达百余种，以软缎和彩丝为主要材料。刺绣工艺是汉服高端定制的核心价值所在。",
         json.dumps({"en":"Four Great Embroideries of China","ja":"中国四大名刺繍","ko":"중국 사대 명자수"}, ensure_ascii=False),
         None,
         json.dumps({"欧洲":"如法国高级定制刺绣工坊般的匠艺","北美":"堪比印第安珠绣的文化传承","东南亚":"与泰国皇家刺绣同为宫廷工艺"}, ensure_ascii=False)),
        ...
        ("形制", "宋代褙子的极简美学",
         ...
         json.dumps({"en":"Song Dynasty Beizi: Minimalist Aesthetics","ja":"宋の褙子—極簡の美学","ko":"송대 배자—극간의 미학"}, ensure_ascii=False),
         None,
         json.dumps({"日韩":"与和服羽织有异曲同工之妙","欧洲":"如Coco Chanel小黑裙般的简约革命","北美":"极简主义设计理念的东方先驱"}, ensure_ascii=False)),
        ...
        ("礼仪", "冠礼 — 华夏男子的成人礼",
         ...
         json.dumps({"en":"Guanli: The Coming-of-Age Ceremony","ja":"冠礼—漢民族男子の成人式","ko":"관례—화하 남자의 성인식"}, ensure_ascii=False),
         None,
         json.dumps({"北美":"如犹太Bar Mitzvah般的成人仪式","欧洲":"如骑士授剑礼般的身份转换","东南亚":"与佛教出家礼同具仪式感"}, ensure_ascii=False)),

        ("纹样", "八宝纹的吉祥密码",
         "八宝纹是中国传统吉祥纹样中的经典组合，源于佛教'八吉祥'（八瑞相），包括宝伞、金鱼、宝瓶、妙莲、右旋白螺、吉祥结、胜利幢、金轮。每件法宝各有寓意：宝伞遮蔽魔障、金鱼象征自由、宝瓶代表财富、莲花寓意纯洁。八宝纹广泛应用于明清织锦和瓷器装饰之中。",
         json.dumps({"en":"Eight Treasures: Auspicious Symbols","ja":"八宝文—吉祥の暗号","ko":"팔보문—길상의 암호"}, ensure_ascii=False),
         None,
         json.dumps({"欧洲":"如基督教圣物象征体系","北美":"如印第安Medicine Wheel的象征系统","中东":"与伊斯兰几何纹样的精神寓意共鸣"}, ensure_ascii=False)),
    ]
